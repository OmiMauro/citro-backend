...
const inscriptionValidator = (req, res, next) => {
  ...
  req.check('registrationCar', 'Debe ingresar la patente del auto con el que participará').notEmpty()
  ...
  req.check('VTV', 'Debe especificar si tiene Verificación Técnica Vehicular').notEmpty()
  req.check('travelPeople', 'Debe especificar cuántas personas viajan en su auto').notEmpty()
  req.check('arrivalDate', 'Debe especificar la fecha aprox que llegará a Misiones').notEmpty()
  req.check('dateToProvince', 'Debe especificar la fecha que regresará a su provincia').notEmpty()
